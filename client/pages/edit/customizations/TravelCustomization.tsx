...
export default function TravelCustomization() {
    ...
    try {
      ...
      if (travelError) {
        if (travelError.code === "PGRST116") {
          ...
          console.log(
            "No travel record found for event:",
            eventId,
            "- using defaults",
          );
          setTravelInfo({
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      if (travelInfo.flight_info !== undefined)
        await saveTravelField("flight_info", travelInfo.flight_info);
      if (travelInfo.accommodations !== undefined)
        await saveTravelField("accommodations", travelInfo.accommodations);
      if (travelInfo.daily_schedule !== undefined)
        await saveTravelField("daily_schedule", travelInfo.daily_schedule);
