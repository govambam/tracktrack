...
export default function PublicLeaderboard() {
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center space-x-2 mb-6">
          ...
          <h3 className="text-2xl font-bold text-slate-900">Overall Stableford Leaderboard</h3>
        </div>
        <p className="text-slate-600 mb-8">Combined points from all rounds (individual + team)</p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${
                    index === 0 ? 'bg-yellow-500 text-white' : 'bg-slate-100 text-slate-700'
                  }`}>
                    {player.position}
                  </div>
                  <div>
                    <div className="flex items-center space-x-2">
                      <span className="font-bold text-slate-900 text-lg">{player.name}</span>
                      {player.badge && (
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold text-green-600">{player.points}</div>
      ...
      <div>
        <div className="flex items-center space-x-2 mb-6">
          ...
          <h3 className="text-2xl font-bold text-slate-900">Team Scramble Scores</h3>
        </div>
      ...
      <div>
        <div className="flex items-center space-x-2 mb-6">
          ...
          <h3 className="text-2xl font-bold text-slate-900">Prize Structure (Final)</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl p-6 border-2 border-yellow-200">
            <div className="flex items-center space-x-3 mb-2">
              ...
              <span className="font-bold text-slate-900">Overall Champion:</span>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 border-2 border-orange-200">
            ...
            <div className="text-sm text-slate-500">Awarded after all rounds complete</div>
          </div>
        </div>
      </div>
    </div>
  ...
  const renderMoneyTab = () => (
    <div className="space-y-8">
      ...
      <p className="text-slate-600 mb-8">Current winnings from skills contests ($10 each)</p>
      
      <div className="space-y-4">
        {mockPlayers.map((player, index) => (
          <div key={player.name} className="bg-white rounded-2xl p-6 border-2 border-slate-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-500">
                    {player.name === 'Ivan' ? 'Scarecrow holes 2' : 'No prizes yet'}
                  </div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-green-600">${player.money}</div>
                <div className="text-sm text-slate-500">
                  {player.name === 'Ivan' ? '1 contests won' : 'No contests won yet'}
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="space-y-8">
      {mockCourses.map((course, courseIndex) => (
        <div key={course.name}>
          ...
          <p className="text-slate-600 mb-6">{course.holes} holes • {course.format}</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-sm font-semibold text-slate-900">Hole</th>
                    {Array.from({ length: Math.min(6, course.holes) }, (_, i) => (
                      <th key={i + 1} className="px-3 py-3 text-center text-sm font-semibold text-slate-900">
                        {i + 1}
                      </th>
                    ))}
                    <th className="px-4 py-3 text-center text-sm font-semibold text-slate-900">Total</th>
                    <th className="px-4 py-3 text-center text-sm font-semibold text-slate-900">Stableford</th>
                  </tr>
                </thead>
                
                <tbody>
                  <tr className="border-b border-slate-200">
                    <td className="px-4 py-3 font-semibold text-slate-900">Par</td>
                    {mockScorecard.map((hole) => (
                      <td key={hole.hole} className="px-3 py-3 text-center font-semibold text-slate-600">
                        {hole.par}
                      </td>
                    ))}
                    <td className="px-4 py-3 text-center font-bold text-slate-900">72</td>
                    <td className="px-4 py-3 text-center font-bold text-slate-900">36</td>
                  </tr>
                  ...
                  {mockPlayers.map((player) => (
                    <tr key={player.name} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      {mockScorecard.map((hole) => {
                        const score = hole.scores[player.name as keyof typeof hole.scores];
                        const par = hole.par;
                        const isGoodScore = score < par;
                        const isBadScore = score > par;
                        
                        return (
                          <td key={hole.hole} className="px-3 py-3 text-center relative">
                      ...
                      <td className="px-4 py-3 text-center font-bold text-slate-900">
                        {courseIndex === 0 ? (player.name === 'Patrick' ? '72' : player.name === 'Ivan' ? '73' : '75') : '-'}
                      </td>
                  ...
                  <tr className="bg-slate-50">
                    <td className="px-4 py-3 font-semibold text-slate-900">Contest</td>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-green-50/30">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-green-600 font-semibold mb-2">& Prize Tracker</p>
        ...
        <div className="text-center mt-16 pt-8 border-t border-slate-200">
          <p className="text-slate-500">Live tournament data powered by TrackTrack Golf</p>
        </div>
      </div>
    </div>
  );
