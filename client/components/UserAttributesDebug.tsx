...
export const UserAttributesDebug: React.FC = () => {
  ...
  const categorizeAttributes = (attrs: any) => {
    ...
    Object.entries(attrs).forEach(([key, value]) => {
      if (['id', 'email', 'emailDomain', 'name', 'fullName', 'handicap', 'hasHandicap', 'handicapRange', 'location', 'bio', 'hasProfileImage', 'isAuthenticated'].includes(key)) {
        categories.user[key] = value;
      } else if (['deviceType', 'browser', 'os', 'userAgent', 'screenWidth', 'screenHeight', 'viewportWidth', 'viewportHeight'].includes(key)) {
        categories.device[key] = value;
      } else if (['timezone', 'country', 'language'].includes(key)) {
        categories.location[key] = value;
      } else if (['totalEvents', 'hasCreatedEvents', 'userType', 'engagementLevel', 'accountAgeInDays', 'accountAgeCategory'].includes(key)) {
  ...
  const renderAttributeSection = (title: string, icon: React.ReactNode, attrs: any) => {
    ...
    <Card className="border-blue-200">
      <CardHeader>
        ...
        <CardDescription className="text-blue-600">
          Current user attributes available for feature flag targeting. Total: {Object.keys(attributes).length} attributes
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          ...
          <div className="text-sm text-blue-700 space-y-1">
            <div>• Target mobile users: <code>deviceType = "mobile"</code></div>
            <div>• Target power users: <code>userType = "power_user"</code></div>
            <div>• Target new accounts: <code>accountAgeCategory = "new"</code></div>
            <div>• Target by handicap: <code>handicapRange = "low"</code></div>
            <div>• Target by time: <code>hourOfDay &gt; 18</code> (evening users)</div>
          </div>
        </div>
      </CardContent>
    </Card>
