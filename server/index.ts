  ...
  app.post("/api/invitations/send", async (req, res) => {
    ...
    try {
      ...
      if (playersError) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to fetch invited players" });
      }
      ...
      const emailResults = [];
      const baseUrl =
        process.env.BASE_URL || req.get("host")
      ...
      for (const player of invitedPlayers) {
        try {
          ...
          if (
            player.invited_email.includes("@placeholder.local") ||
            player.invited_email.includes("@example.com")
          ) {
            console.log(
              `⏭️ Skipping placeholder email: ${player.invited_email}`,
            );
            continue;
          }
          ...
          console.error(
            `❌ Error sending email to ${player.invited_email}:`,
            emailError,
          );
          emailResults.push({
            player_id: player.id,
