...
async function testInvitationSystem() {
  ...
  try {
    ...
    const hasCreatedBy = eventsColumns?.some(col => col.column_name === 'created_by');
    ...
    const hasInviteFunction = functions?.some(func => func.routine_name === 'invite_player_to_event');
    const hasAcceptFunction = functions?.some(func => func.routine_name === 'accept_event_invitation');
    
    console.log(`   ✅ invite_player_to_event function exists: ${hasInviteFunction}`);
    console.log(`   ✅ accept_event_invitation function exists: ${hasAcceptFunction}`);
    ...
    console.log("   💡 Use the API endpoints with proper auth tokens for full testing");
    ...
    expectedIndexes.forEach(indexName => {
      ...
      console.log(`   ✅ Index ${indexName}: ${hasIndex ? 'exists' : 'missing'}`);
    });

    console.log("\n🎉 Schema validation complete!");
    console.log("\n📋 Next Steps:");
    console.log("   1. Apply the schema: Run add_event_invitations_schema.sql in Supabase");
    ...
    console.log("   Run the SQL migration first: add_event_invitations_schema.sql");
  }
}
